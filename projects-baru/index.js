/**
   * Create By adn.
   * Contact Me on wa.me/289652903288
   * Follow 
*/
...
async function startHisoka() {
    ...
    const hisoka = hisokaConnect({
        ...
        browser: ['WcB Multi Device','Safari','1.0.0'],
        auth: state,
        version
    })
...
console.log(color('Instagram :', 'cyan'), color(`@niadanxx`, 'yellow'))
console.log(color('Github :', 'cyan'), color(`@ItsmeHndrr`, 'yellow'))
console.log(color('Wangsap :', 'cyan'), color(`0896-5290-3288`, 'yellow'))
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./adnn")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
cangtion = `${num.split("@")[0]}, jangan lupa nitip nasbung 5 buat di group` 
...
let btn = [{
                                urlButton: {
                                    displayText: 'Script',
                                    url: 'https://wa.me/6289652903288?text=mau+beli+scnya+kak'
                                }
                            }, {
                                quickReplyButton: {
                                    displayText: 'Welcome👋',
...
Kon = await getBuffer(`https://hardianto.xyz/api/tools/welcomer?nama=${encodeURIComponent(nama)}&namaGb=${encodeURIComponent(metadata.subject)}&pepeGb=${encodeURIComponent(ppgroup)}&totalMem=${encodeURIComponent(memb)}&pepeUser=${encodeURIComponent(ppuser)}&bege=https://telegra.ph/file/aa32a5fdf62f400c901a4.jpg&apikey=hardianto`)

Tol = await getBuffer(`https://hardianto.xyz/api/tools/leave?nama=${encodeURIComponent(nama)}&namaGb=${encodeURIComponent(metadata.subject)}&pepeGb=${encodeURIComponent(ppgroup)}&totalMem=${encodeURIComponent(memb)}&pepeUser=${encodeURIComponent(ppuser)}&bege=https://telegra.ph/file/aa32a5fdf62f400c901a4.jpg&apikey=hardianto`)
                if (anu.action == 'add') {
